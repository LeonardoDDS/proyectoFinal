...
public class RegistroClienteController {
    ...
    public String mostrarFormularioModificar(@RequestParam(value = "idCliente", required = false) Long id, Model model) {
        ...
        if (id == null) {
            ...
            if (clienteOptional.isPresent()) {
                model.addAttribute("cliente", clienteOptional.get());       // ← cliente a editar
                model.addAttribute("mostrarFormularioEdicion", true);       // ← muestra form de edición
            } else {
                model.addAttribute("error", "Cliente no encontrado con ID: " + id);
                model.addAttribute("mostrarFormId", true);                  // ← deja el formulario visible
            }
        }
    ...
    public String guardarCliente(@ModelAttribute ClienteEntity cliente) {
        ...
        return "redirect:/formularioPago"; // O redirige donde tú quieras
    }
